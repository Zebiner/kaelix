...
impl CpuSet {
    ...
    pub fn apply_to_current_thread(&self) -> RuntimeResult<()> {
        ...
        unsafe {
            ...
            if result != 0 {
                return Err(crate::runtime::RuntimeError::CpuAffinity(
...
impl NumaTopology {
    ...
    fn detect_numa_nodes() -> RuntimeResult<(Vec<NumaNode>, HashMap<usize, NodeId>)> {
        ...
        for entry in entries {
            let entry = entry.map_err(|e| crate::runtime::RuntimeError::NumaTopology(e.to_string()))?;
        ...
        for range in cpulist.split(',') {
            ...
            if let Some(dash_pos) = range.find('-') {
                let start: usize = range[..dash_pos].parse()
                    .map_err(|e| crate::runtime::RuntimeError::NumaTopology(
...
mod tests {
    ...
    fn test_parse_cpulist() {
        assert_eq!(
            NumaTopology::parse_cpulist("0-3").unwrap(),
            vec![0, 1, 2, 3]
        );
        
        assert_eq!(
            NumaTopology::parse_cpulist("0,2,4").unwrap(),
            vec![0, 2, 4]
        );
        
        assert_eq!(
            NumaTopology::parse_cpulist("0-1,4-5").unwrap(),
            vec![0, 1, 4, 5]
        );
        
        assert_eq!(
            NumaTopology::parse_cpulist("8").unwrap(),
            vec![8]
        );
    }
